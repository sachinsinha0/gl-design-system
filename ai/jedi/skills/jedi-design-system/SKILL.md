...
description: >
  Use when building, editing, or reviewing any UI for Jedi — Great Learning's internal staff & faculty admin tools,
  built on MUI v6 + React (package @gl/jedi). Bundles Jedi's design rules and Great Learning's content
  rules (voice, casing, punctuation, component copy, error/empty states, SEO). Trigger before
  generating any Jedi screen, component, or user-facing string, and before writing or reviewing Jedi copy.
---

# Jedi Design System — AI Skill

Jedi is Great Learning's design system for the **internal staff & faculty admin tools**.
Stack: **MUI v6 + React** · Package: `@gl/jedi`.

Generated UI is correct only when it satisfies **both** the design rules and the content rules below.

## Part 1 — Jedi design rules (summary)

Import from @gl/jedi never @mui/material · colors via getColors(mode) · flat & outlined (no elevated cards) · closed type/spacing scales (no h6, no 13/15/17px) · UPPERCASE buttons.
...
- Full spec: `jedi-design-system.md`
- Lean paste-in version: `jedi-design-system.context.md`
...
Every user-facing string in Jedi follows these rules. They are identical across Magna, Jedi and GLDS-Web — only which sections apply differs.

**Applies to Jedi:** Sections A–H apply to all admin copy. Section I (Olympus) applies directly — esp. student-vs-faculty labels (I.2) & the assignment status table (I.3). Section J (SEO) does NOT apply.
