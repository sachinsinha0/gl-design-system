...
description: >
  Use when building, editing, or reviewing any UI for Magna — Great Learning's learner-facing student app,
  built on Tamagui + React-Native-Web · Material 3 (package @gl/elements). Bundles Magna's design rules and Great Learning's content
  rules (voice, casing, punctuation, component copy, error/empty states, SEO). Trigger before
  generating any Magna screen, component, or user-facing string, and before writing or reviewing Magna copy.
---

# Magna Design System — AI Skill

Magna is Great Learning's design system for the **learner-facing student app**.
Stack: **Tamagui + React-Native-Web · Material 3** · Package: `@gl/elements`.

Generated UI is correct only when it satisfies **both** the design rules and the content rules below.

## Part 1 — Magna design rules (summary)

Use M3 role tokens ($primary/$onSurface) never raw hex · Title-Case buttons · Lucide icons only · 48dp targets · body ≥14px · untyped so render tests are the safety net.
...
- Full spec: `magna-design-system.md`
- Lean paste-in version: `magna-design-system.context.md`
...
Every user-facing string in Magna follows these rules. They are identical across Magna, Jedi and GLDS-Web — only which sections apply differs.

**Applies to Magna:** Sections A–H apply to all in-app copy. Section I (Olympus) applies to student surfaces. Section J (SEO) does NOT apply.
